...
st.set_page_config(page_title="AgriSight Analytics | INF 232", layout="wide")
...
if st.session_state.donnees.empty:
    st.info("Ajoutez des donnees via le formulaire dans la barre laterale ou importez un fichier CSV.")
    ...
    if st.button("Charger les donnees de demonstration"):
        ...
        donnees_demo = pd.DataFrame({
            ...
            'Date': [str(d)[:10] for d in pd.date_range('2026-01-01', periods=n, freq='5D')],
        ...
        donnees_demo['Rendement_t_ha'] = (
            2.5 + 0.1 * donnees_demo['Temperature_C'] 
            - 0.02 * donnees_demo['Humidite_Pct'] 
            + 0.5 * donnees_demo['pH_Sol'] 
            + 0.01 * donnees_demo['Precipitations_mm'] 
            + 0.02 * donnees_demo['Engrais_kg_ha'] 
    ...
    with onglet1:
        ...
        st.metric("Rendement moyen (t/ha)", f"{pd.to_numeric(df['Rendement_t_ha'], errors='coerce').mean():.2f}")
        ...
        fig_corr = px.imshow(df_num.corr(), text_auto=".2f", aspect="auto", title="Matrice de correlation", color_continuous_scale='RdBu_r', zmin=-1, zmax=1)
    ...
    with onglet2:
        ...
        var_independante = st.selectbox(
            "Selectionner la variable independante (X)",
        ...
        if len(X) > 1:
            ...
            fig.add_trace(go.Scatter(x=x_range.flatten(), y=y_range, mode='lines', name=f'Regression (R2={r2:.3f})', line=dict(color='darkgreen', width=3)))
    ...
    with onglet3:
        ...
        if len(df_model) > len(colonnes_caract):
            ...
            fig_reel = px.scatter(
                ...
                labels={'x': 'Rendement Reel', 'y': 'Rendement Predit'},
            ...
            st.warning(f"Il faut au moins {len(colonnes_caract)+1} echantillons pour une regression multiple fiable. Actuellement : {len(df_model)}.")
    ...
    with onglet4:
        ...
        df_acp['Rendement_t_ha'] = pd.to_numeric(df_acp['Rendement_t_ha'], errors='coerce')
        ...
        if len(df_acp) > 2:
            ...
            fig_acp = px.scatter(
                ...
                title="Visualisation ACP (Coloree par le Rendement)",
    ...
    with onglet5:
        ...
        df_clust['Rendement_t_ha'] = pd.to_numeric(df_clust['Rendement_t_ha'], errors='coerce')
        ...
        if len(df_clust) > 3:
            ...
            with col2:
                ...
                profil_cluster = df_clust.groupby('Cluster')[['Temperature_C', 'Humidite_Pct', 'pH_Sol', 'Rendement_t_ha']].mean()
                st.dataframe(profil_cluster.style.format("{:.1f}"), use_container_width=True)
            
            st.subheader("Distribution du rendement par zone")
            fig_box = px.box(
                df_clust, x='Cluster', y='Rendement_t_ha',
                color='Cluster',
...
st.markdown("---")
st.markdown("**INF 232 EC2 - TP1** | Application de collecte et d'analyse descriptive des donnees")